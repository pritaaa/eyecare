...
class MainActivity : FlutterActivity() {
    ...
    private fun getUsageStats(targetPackages: List<String>?): List<Map<String, Any?>> {
        ...
        // 1. Ambil raw data dari helper function (Stack Logic)
        val rawUsageMap = getUsageMap(startTime, endTime)
        val excludedPackages = getExcludedPackages()
        val packageTrackableCache = HashMap<String, Boolean>()

        val resultList = if (targetPackages != null && targetPackages.isNotEmpty()) {
            targetPackages.map { pkg ->
                val total = rawUsageMap[pkg] ?: 0L
                AbstractMap.SimpleEntry(pkg, total)
            }
        } else {
            // Filter aplikasi yang tidak dilacak (Launcher, System UI, dll)
            rawUsageMap.entries
                .filter { (pkg, _) ->
                    packageTrackableCache.getOrPut(pkg) { isTrackableApp(pkg, excludedPackages) }
                }
    ...
    private fun calculateTotalUsage(startTime: Long, endTime: Long): Long {
        ...
        // 1. Ambil raw data yang sama persis dengan getUsageStats
        val rawUsageMap = getUsageMap(startTime, endTime)
        val excludedPackages = getExcludedPackages()
        val packageTrackableCache = HashMap<String, Boolean>()
        
        // 2. Jumlahkan hanya aplikasi yang valid (Trackable)
        var totalMs = 0L
        for ((pkg, duration) in rawUsageMap) {
            val isTrackable = packageTrackableCache.getOrPut(pkg) {
    ...
    // Helper function: Menggunakan logika Stack untuk menghitung durasi setiap aplikasi
    // Memproses SEMUA event (termasuk launcher) agar stack tetap akurat,
    // tapi filtering dilakukan setelahnya oleh pemanggil.
    private fun getUsageMap(startTime: Long, endTime: Long): Map<String, Long> {
        val usageStatsManager = getSystemService(Context.USAGE_STATS_SERVICE) as UsageStatsManager
        ...
        while (events.hasNextEvent()) {
            ...
            // Atribusikan waktu ke aplikasi paling atas di stack
            if (stack.isNotEmpty()) {
                val topPkg = stack.last()
                val duration = currentTime - lastEventTime
                if (duration > 0) {
                    usageMap[topPkg] = usageMap.getOrDefault(topPkg, 0L) + duration
                }
            }
